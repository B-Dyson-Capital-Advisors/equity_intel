...
def get_most_recent_lawyer_from_filing(cik, company_name, firm_name, adsh, debug=False):
    ...
    try:
        ...
        # 1. Complete submission text file (NO JAVASCRIPT - contains everything!)
        # 2. Direct .htm file (often 404)
        # 3. Viewer API (requires JavaScript - not parseable)
        doc_urls = [
            f"https://www.sec.gov/Archives/edgar/data/{cik_stripped}/{adsh_clean}.txt",
